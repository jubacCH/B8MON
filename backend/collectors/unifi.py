"""
UniFi Network integration – collects device health, client stats, and WAN metrics
from a UniFi Controller or UniFi OS (Dream Machine) via cookie-based REST API.
"""
...
async def import_unifi_devices(ctrl_name: str, data: dict, db) -> dict:
    """
    Auto-import UniFi network devices (APs, switches, gateways) as PingHosts.

    Matches existing hosts by IP address or MAC address. Only imports devices
    that have an IP address. Returns {"added": int, "merged": int, "skipped": int}.
    """
    ...
    for d in data.get("devices", []):
        ...
        if existing:
            ...
            new_host = PingHost(
                ...
                hostname=ip,
                check_type="icmp",
                enabled=d.get("state", 0) == 1,
                source="unifi",
                source_detail=ctrl_name,
...
class UnifiAPI:
    ...
    async def _session(self) -> tuple[httpx.AsyncClient, dict]:
        """Login and return (client, headers). Caller must close the client."""
        client = httpx.AsyncClient(verify=self.verify_ssl, timeout=15.0, follow_redirects=True)
    ...
    async def _get(self, client: httpx.AsyncClient, headers: dict, path: str) -> list:
        resp = await client.get(f"{self._api_base}{path}", headers=headers)
        resp.raise_for_status()
        return resp.json().get("data", [])

    async def fetch_all(self) -> dict:
        """Fetch devices, clients, health, and events in one session. Returns parsed dict."""
        async with httpx.AsyncClient(verify=self.verify_ssl, timeout=15.0, follow_redirects=True) as client:
            ...
            # Events endpoint may not exist on all firmware versions – ignore errors
...
_EVENT_KEY_LABELS = {
    "EVT_WU_Connected":         "Client connected",
    "EVT_WU_Disconnected":      "Client disconnected",
    "EVT_WU_Roam":              "Client roamed",
    "EVT_LU_Connected":         "Wired client connected",
    "EVT_LU_Disconnected":      "Wired client disconnected",
    "EVT_AP_Lost_Contact":      "AP lost contact",
    "EVT_AP_Adopted":           "AP adopted",
    "EVT_AP_Connected":         "AP connected",
    "EVT_AP_Deleted":           "AP removed",
    "EVT_AP_Restarted":         "AP restarted",
    "EVT_AP_Upgraded":          "AP firmware upgraded",
    "EVT_SW_Lost_Contact":      "Switch lost contact",
    "EVT_SW_Connected":         "Switch connected",
    "EVT_SW_Adopted":           "Switch adopted",
    "EVT_GW_Lost_Contact":      "Gateway lost contact",
    "EVT_GW_Connected":         "Gateway connected",
    "EVT_GW_WAN_Transition":    "WAN status changed",
...
def parse_unifi_data(raw_devices: list, raw_clients: list, raw_health: list,
    ...
    """
    Parse raw UniFi API responses into a structured summary.
    ...
    for d in raw_devices:
        ...
        clients_wifi = sum(
            r.get("num_sta", 0) for r in d.get("radio_table_stats", [])
        )
        # Wired port client count
        ...
        if has_ports:
            for p in d.get("port_table", []):
                ...
                port_table.append({
                    ...
                    "rx_bytes_r": p.get("rx_bytes-r", 0) or 0,
                    "tx_bytes_r": p.get("tx_bytes-r", 0) or 0,
                    "rx_bytes":   p.get("rx_bytes", 0) or 0,
                    "tx_bytes":   p.get("tx_bytes", 0) or 0,
                    "satisfaction": p.get("satisfaction", -1),
                    "op_mode":    p.get("op_mode", ""),   # switching / aggregate / mirror / disabled
                })
        ...
        devices.append({
            ...
            "type":         dtype,
            "type_label":   _DEVICE_TYPE_LABELS.get(dtype, dtype.upper()),
            "model":        d.get("model", ""),
            ...
            "cpu_pct":      cpu_pct,
            "mem_pct":      mem_pct,
            "clients_wifi":  max(clients_wifi, 0),
            "clients_wired": max(clients_wired, 0),
            "rx_bytes":     d.get("rx_bytes", 0) or 0,
            "tx_bytes":     d.get("tx_bytes", 0) or 0,
            "rx_bytes_r":   d.get("uplink", {}).get("rx_bytes-r", 0) or 0,
            "tx_bytes_r":   d.get("uplink", {}).get("tx_bytes-r", 0) or 0,
            "satisfaction": d.get("satisfaction", -1),
            "version":      d.get("version", ""),
            "port_table":   port_table,
            "has_ports":    has_ports and len(port_table) > 0,
        })
    ...
    for c in raw_clients:
        ...
        clients.append({
            ...
            "rx_bytes_r":   c.get("rx_bytes-r", 0) or 0,
            "tx_bytes_r":   c.get("tx_bytes-r", 0) or 0,
            "uptime":       c.get("uptime", 0),
            "is_wireless":  is_wireless,
            "vlan":         c.get("vlan", 1) or 1,
            "ssid":         c.get("essid", ""),
            "satisfaction": c.get("satisfaction", -1),
    ...
    for h in raw_health:
        ...
        if sub == "wan":
            wan = {
                ...
                "rx_bytes_r": h.get("rx_bytes-r", 0) or 0,
                "tx_bytes_r": h.get("tx_bytes-r", 0) or 0,
            }
        elif sub == "lan":
            lan = {
                "status":           h.get("status", "unknown"),
                "num_adopted":      h.get("num_adopted", 0),
                "num_disconnected": h.get("num_disconnected", 0),
    ...
    for e in (raw_events or [])[:200]:
        ...
        events.append({
            ...
            "msg":       e.get("msg", ""),
            "key":       key,
            "key_label": _EVENT_KEY_LABELS.get(key, key.replace("EVT_", "").replace("_", " ").title()),
            "subsystem": sub,
            "sub_label": _EVENT_SUBSYSTEM_LABELS.get(sub, sub.upper() or "System"),
