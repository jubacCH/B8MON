...
# Map NUT status tokens to human labels
_STATUS_LABELS = {
    "OL":  "On Line",
    "OB":  "On Battery",
    "LB":  "Low Battery",
...
def parse_nut_vars(lines: list[str]) -> dict:
    """
    Parse VAR lines like: VAR ups battery.charge "100"
    Returns a structured dict with key UPS metrics.
    """
    raw: dict[str, str] = {}
    for line in lines:
    ...
    # Status can be compound, e.g. "OL CHRG"
    ...
    return {
        ...
        "temp":           temp,
        "power_w":        power_w,
        "manufacturer":   raw.get("ups.mfr", ""),
        "model":          raw.get("ups.model", ""),
...
class NutClient:
    ...
    async def fetch_all(self) -> dict:
        """Connect to NUT daemon and fetch UPS variables."""
    ...
    async def health_check(self) -> bool:
        """Return True if the UPS is reachable and responding."""
        try:
